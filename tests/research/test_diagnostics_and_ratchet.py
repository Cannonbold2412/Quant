...
from aqrl.research import db, evaluate
...
class _WindowResult:
    def __init__(self, folds, score=0.6):
        ...
        self.concatenated_dates = np.datetime64("2015-01-01") + np.arange(n_bars).astype("timedelta64[D]")
...
RESULTS_DIAG_KEYS = [c for c in evaluate.RESULTS_COLUMNS if c not in ("commit", "status", "description")]
...
def _check(score, threshold, ratcheted):
    return evaluate._bar_check(
        score,
        n_trades=evaluate.MIN_TRADES,
        max_dd=0.05,
        oos_returns=_OK_RETURNS,
        threshold=threshold,
        ratcheted=ratcheted,
    )
...
def test_validity_floors_outrank_any_score():
    ...
    assert evaluate._bar_check(
        0.99,
        n_trades=evaluate.MIN_TRADES - 1,
        max_dd=0.01,
        oos_returns=_OK_RETURNS,
        threshold=0.0,
        ratcheted=True,
...
def test_tabs_in_a_description_cannot_shift_later_columns(tmp_path):
    path = tmp_path / "results.tsv"
    db.append_results_tsv(path, evaluate._row("c", "crash", "boom\there\nand here"), evaluate.RESULTS_COLUMNS)
